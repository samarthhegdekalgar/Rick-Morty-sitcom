...
const CharacterContainer: FC<CharacterContainerProps> = ({
  ...
  return (
    <img
      ...
      alt={name}
      onClick={() => {
        contextState.setCharacter({
          basicInfo: detailedObject,
          location: undefined,
          origin: undefined,
          episode: undefined,
        })
      ...
      className={`${
        index % 2 === 0 ? 'col-span-2 row-span-2' : 'col-span-1'
      } place-self-stretch rounded-md hover:cursor-pointer border-2 border-transparent hover:border-gray-500 focus:outline-none`}
    />
  )
}
