 ...
 */
const Layout: FC<LayoutProps> = ({ mainContent, sliderContent, footerContent }: LayoutProps) => {
    // The state of the collapsed sider.
    const [collapsed, setCollapsed] = useState(true);

    return (
        <AntdLayout className={'text-white h-screen overflow-clip bg-slate-900'}>
            <AntdLayout className="h-full">
                <Content className={`${collapsed ? 'w-full' : 'w-[60%] opacity-50 pointer-events-none'}  h-full transition-all`} onClick={() => setCollapsed(!collapsed)}>{mainContent}</Content>
            ...
            <Footer className="bg-slate-800 w-full h-10 grid place-content-center bottom-0 absolute">{footerContent}</Footer>
        </AntdLayout>
    );
}
