    ...
    return (
        <AntdLayout className={'text-white overflow-clip bg-slate-900 p-2'}>
            <AntdLayout className="h-full">
                ...
                <Sider
                    className={`h-full bg-white absolute top-0 right-0 z-10 transition-all`}
                    width={collapsed ? '0%' : isSmallScreen ? '100%' : '40%'}
                >
                    <div className="flex flex-col">
                        <div className="py-3 px-4 border-b border-slate-200 flex items-center">
                            <button onClick={() => setCollapsed(true)} className="focus:outline-none">
                                <svg viewBox="64 64 896 896" focusable="false" data-icon="close" className="h-8 w-8 text-gray-400" fill="currentColor" aria-hidden="true">
