...
const CharacterDetail: FC = () => {
  ...
  useEffect(() => {
    ...
    let newOrigin: Location | undefined
    let newEpisode: Episode[] | Episode | undefined
    const getUpdatedValue = async (): Promise<{
      location?: Location
      origin?: Location
      episode?: Episode[]
    }> => {
      if (
        character?.basicInfo.location.url &&
        character.basicInfo.location.url === character.basicInfo.origin.url &&
        character.basicInfo.location.name !== 'unknown'
      ) {
      ...
      return {
        ...
        episode: Array.isArray(newEpisode) ? newEpisode : newEpisode ? [newEpisode] : undefined,
        origin: newOrigin,
      }
    }
    ...
    getUpdatedValue()
      .then((response) => {
        if (character) setCharacter({ ...character, ...response })
      })
      .catch(console.error)
            ...
            <span className="absolute bottom-0 right-0 transform translate-y-1/2 translate-x-1/2 block border-2 border-white rounded-full">
              <span
                className={`block h-4 w-4 rounded-full ${
                  character?.basicInfo.status === 'Alive'
                    ? 'bg-green-400'
                    : character?.basicInfo.status === 'Dead'
                    ? 'bg-red-400'
                    : 'bg-gray-400'
                }`}
              />
